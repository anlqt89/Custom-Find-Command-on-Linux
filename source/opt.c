...
int isFound(char *where, char *subName,  char *name, char *mmin, char *inum){
	...
	//if name!= NULl, then find with name
			//if name exist, then if mmin exist find with name & mmin, else find with only name
		//else find with only mmin
		//else find with only inode number	
	if(name != NULL ){
		...
		while((de = readdir(dir)) != NULL){
			...
			if(strcmp(de->d_name, currDir ) != 0 && strcmp(de->d_name, prevDir) != 0){		
				...
				if(name!= NULL || mmin!= NULL || inum != NULL){	
					...
					if(isFound(fullpath, de->d_name, name, mmin, inum) != 0){
